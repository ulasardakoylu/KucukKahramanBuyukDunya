...
public partial class PointsGlass : Area2D
{
    private AnimatedSprite2D animatedSprite;
    private int pointValue = 1;  // Her Glass 1 puan
    ...
    private void OnBodyEntered(Node2D body)
    {
        ...
        {
            // Player'a Glass puanı ekle
            if (body.HasMethod("AddGlass"))
            {
                body.Call("AddGlass", pointValue);
            }
    ...
    {
        if (player.HasMethod("AddGlass"))
        {
            player.Call("AddGlass", pointValue);
        }
