...
public partial class Level2 : Node2D
{
    ...
    private void LevelPassed()
    {
        ...
        {
            if (ResourceLoader.Exists("res://Assets/Scenes/Areas/Level3.tscn"))
                GetTree().ChangeSceneToFile("res://Assets/Scenes/Areas/Level3.tscn");
            else
    ...
    private void SaveLevelScore()
    {
        try
        {
            ...
            // Level 2 ID = 2
            bool success = Database.SaveScore(userId, 2, currentLevelScore);

            if (success)
                GD.Print($"[LEVEL] ✅ Skor kaydedildi: User={userId}, Level=2, Score={currentLevelScore}");
            else
